...
func TestIntegration_GenerateForChanges_Base(t *testing.T) { //nolint:maintidx,lll // Low Maintainability due to huge test dataset
	t.Parallel()

	expected, err := os.ReadFile("./testdata/golden-base_summary.md")
	if err != nil {
		t.Fatalf("Diff() error while reading previous lock file = %v", err)
	}
	...
	current := summary.GenerateForChanges(changes)

	if string(expected) != current {
		t.Fatalf("unexpected output: diff %s", diff.LineDiff(string(expected), current))
	}
}
