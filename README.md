...
`deps-diff` is a Go module that analyzes and compares two PHP Composer lock files to detect added, removed, and updated package dependencies. It can optionally use `composer.json` files to determine which packages are direct requirements.

## Installation

```bash
go get github.com/user/deps-diff
```
...
import (
    ...
    "github.com/user/go-deps-diff/diff"
)
